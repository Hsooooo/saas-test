...
import com.illunex.emsaasrestapi.member.entity.*;
import com.illunex.emsaasrestapi.member.repository.*;
...
public class MemberService {
    ...
    public CustomResponse<?> login(HttpServletRequest request, HttpServletResponse response, RequestMemberDTO.Login login) throws CustomException {
        ...
        if(passwordEncoder.matches(login.getPassword(), member.getPassword())) {
            ...
            memberLoginHistory.setMemberIdx(member.getIdx());
            memberLoginHistory.setBrowser(MemberComponent.getClientPlatform(request));
            memberLoginHistory.setPlatform(MemberComponent.getClientDevice(request));
            memberLoginHistory.setIp(MemberComponent.getClientIpAddr(request));
    ...
    public CustomResponse<?> join(RequestMemberDTO.Join joinData) throws Exception {
        ...
        if(memberRepository.findByNickname(joinData.getNickname()).isPresent()) {
            // 중복된 닉네임이 있을 경우
        ...
        // 회원 등록
        Member member = modelMapper.map(joinData, Member.class);
        // 일반 회원
    ...
    public CustomResponse<?> checkDuplicate(String type, String value) throws CustomException {
        switch(type) {
            case "email" :
                ...
                break;
            case "nickname" :
                if(memberRepository.findByNickname(value).isPresent()) {
                    throw new CustomException(ErrorCode.MEMBER_NICKNAME_DUPLICATE);
                }
                break;
            default:
